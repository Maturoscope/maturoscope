...
export async function POST(request: NextRequest) {
  try {
    ...
    const { gaps, contactInformation, projectName } = body

    if (!gaps || !contactInformation || !projectName) {
    ...
    // Call backend API
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL}/services/contact?organizationKey=${organizationKey}`
    ...
    console.error('Error in /api/services/contact:', error)
