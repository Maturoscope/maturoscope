...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-gray-900/50 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:duration-300 data-[state=open]:duration-300",
      className
    )}
    {...props}
    ref={ref}
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg gap-4 border border-border/20 bg-background p-6 shadow-2xl rounded-lg",
          "transition-all duration-300 ease-out -translate-x-1/2",
          isMounted 
            ? "opacity-100 scale-100 -translate-y-1/2" 
            : "opacity-0 scale-90 -translate-y-[calc(50%-1rem)]",
          className
        )}
        {...props}
      >
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Cancel
    ...
    className={cn(
      ...
      "mt-2 sm:mt-0 hover:scale-105 active:scale-95",
      className
    )}
    {...props}
  />
