...
const logger = createStructuredLogger('languages/services-status');
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/languages/services-status`, {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to fetch translation status' },
