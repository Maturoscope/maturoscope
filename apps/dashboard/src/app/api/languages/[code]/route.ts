...
const logger = createStructuredLogger('languages/[code]');
...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/languages/${encodeURIComponent(code)}`, {
      method: 'PATCH',
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to update language' },
