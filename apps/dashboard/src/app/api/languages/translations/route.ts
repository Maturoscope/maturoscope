...
const logger = createStructuredLogger('languages/translations');
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(
      `${apiBaseUrl}/languages/translations${search ? `?${search}` : ''}`,
      { headers: buildApiHeaders(request, token.value), signal: controller.signal },
    );
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to fetch translations' },
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/languages/translations`, {
      method: 'PUT',
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || data.error || 'Failed to save translations' },
        { status: response.status },
      );
    }
    return NextResponse.json(data);
